...
function has888(starts) {
  const values = new Set(starts);
  for (const start of values) {
    if (values.has(start + 16) && values.has(start + 32)) return true;
  }
  return false;
}
...
export function validatePlannerSchedule(plannerState) {
  ...
  for (const assignment of schedule.assignments) {
    ...
    if (conditionMatches(person, "off", assignment.date, assignment.shift)) {
      hardViolations.push(
        violation(
          "HARD_OFF_ASSIGNED",
          "HARD",
          `${person.name} ถูกจัดในช่วง OFF บังคับ`,
    ...
    if (conditionMatches(person, "avoid", assignment.date, assignment.shift)) {
      softViolations.push(
        violation(
          "AVOID_ASSIGNED",
          "SOFT",
          `${person.name} ถูกจัดในช่วงไม่ Prefer`,
          { personId: person.id, date: assignment.date, shift: assignment.shift, slotKey: assignment.key }
        )
      );
    }
  ...
  for (const person of people) {
    ...
    if (state.settings?.avoid888 && has888(stat.starts)) {
      softViolations.push(
        violation(
          "REPEATED_8_HOUR_REST_PATTERN",
          "SOFT",
          `${person.name} มีรูปแบบเวรที่พักเพียง 8 ชั่วโมงซ้ำต่อเนื่อง`,
          { personId: person.id }
        )
      );
    }
