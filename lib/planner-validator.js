...
function personCap(state, person) {
  ...
  if (state.mode === "staff") {
    const globalCap = Number(state.settings?.staffMaxDuty ?? 20);
    return Number.isFinite(globalCap) ? Math.max(0, globalCap) : 20;
  }
...
export function validatePlannerSchedule(plannerState) {
  ...
  for (const person of people) {
    ...
    if (mode === "resident") {
      ...
      const actualCap = personCap(state, person);
      if (stat.actual > actualCap) {
        hardViolations.push(
          violation(
            "STAFF_MAX_ACTUAL_EXCEEDED",
            "HARD",
            `${person.name} มีเวรจริง ${stat.actual} เวร เกินเพดาน ${actualCap} เวร`,
            { personId: person.id, actual: stat.actual, cap: actualCap }
          )
        );
      }
